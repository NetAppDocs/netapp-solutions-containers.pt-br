...
. Existem arquivos de backend de amostra disponíveis no arquivo de instalação baixado no `sample-input` hierarquia de pastas.  Para sistemas NetApp ONTAP que atendem NFS, copie o `backend-ontap-nas.json` arquivo para seu diretório de trabalho e edite o arquivo.
+
[listing]
----
...
NOTE: Existe um campo opcional chamado `fsType` que é definido neste arquivo.  Esta linha pode ser excluída em backends NFS.
